...
function chkWebviewType () {
  if (isApp('jd')) {
    if (!isIOS()) {
      ...
      let tempIosVer = getIOSVersion()
      if (tempIosVer === 'unknown') {
        return 'unknown'
      } else if (tempIosVer >= 8 && tempIosVer < 9) {
        return 'ios8'
      } else {
        let iswk = (-1 !== navigator.userAgent.indexOf('supportJDSHWK/1')) || (window._is_jdsh_wkwebview === 1)
        return iswk ? 'wk' : 'ui'
      }
    }
  }
...
class AudioHelper {
  ...
  create (audioArr) {
    ...
    for (let y = 0; y < audioArr.length; y++) {
  ...
  play (index) {
    ...
    document.querySelector('#' + audioArr[index]['id'])
    this.pause()
  ...
  pause () {
    ...
    if (typeof playIndex != 'undefined') {
      document.querySelector('#' + audioArr[playIndex]['id']).pause()
    }
    // 先判断要通知给原生的state，保证站外也能用state判断
...
const ah = new AudioHelper()

export default {
  install (Vue) {
    Vue.prototype.$audioCreator = ah
    Vue.audioCreator = ah
  },
  $audioCreator: ah
}
